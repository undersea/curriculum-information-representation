...
choice.  Most of the applications challenges were problems which did
not have to interact with proprietary applications.  The one it did
have to deal with was Microsoft Access.  The other problem was how to
make it viewable on a proprietary operating system like Microsoft
Windows.
** Web
...
** Python
   ...
   library.  It is needed by Django \cite{django1}, and is easy to
   read, so easy maintainence. 
...
*** Problems with pyodbc
    The library pyodbc had problems however, but with some
    modifications to the code, it was possible to rely on the modified
    ...
    The problem stemmed from the functions
    that get data from the results of the query and convert them into the
    appropriate data type for python.  The entire library depends on unix
    ODBC C libary.  Its use of the function SQLGetData(...) is flawed, in
    ...
    In the environment that it was being run, it did
    not behave in the way, one assumes, that the author of the code
    thought.  This gave us results like, if the database entry was a
    integer of value 290 the returned result was 3160370, or a double
    value of 40.0 being returned as 6.9524415266644334e-310.


    We found that getting the data as a string value gave us a accurate
    answer represented in a string format.  One could then convert this to
    the appropriate type with a Python convenience C function like for
    integers like PyLong_FromString(...).  
