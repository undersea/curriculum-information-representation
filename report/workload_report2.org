...
#+LaTeX_HEADER: \institute{Computer Science project, Massey University}
...
The data sources are outlined, indicating that not all sources are
used because of access rights being denied to us.  A source that
...
are outlined, showing views specific to them, and what actions the
user is able to take.
...
In conclusion the different users needs
...
full these needs.  The challenge that each user sees the data
in a way appropriate to them, have been meet through permissions.
This is based upon the group that is belonged to, and has the access rights to
...
* Introduction
  ...
  of data sets).  In this project we considered how to implement
  software for a workload presentation system.  The challenge that
  each user sees the data in a way appropriate to them, and had the
  access rights to interact with the data set as required.
...
** Introduction
   ...
   spread the workload fairly, amongst the staff.  It is a artificial
 ...
 - For each paper offered
   ...
   + For each person teaching this paper how much of the paper is
     taught by them.
 ...
 - For each staff member.
   + What postgraduates are being supervised.
     - What percentage of the supervision is being given.
     - Given that each postgraduate degree is worth different hours.
       + What percentage of supervision is given and thus how many of
         those hours is gained.
 - The application must be configurable.
   + This is to be done by an expert, and it doesn't have to be addressed
     in the user interface, just a simple configuration file.

   A spreadsheet application, shown in figure
   ...
   central database where everyone enters information into from
...
***** Ideal world
    Figure \ref{fig:dataflow2} shows a general flow of where data
    comes from and what is being output.  The original idea was to get
    ...
    workload database contains the teaching allocation data,
    and paper coordinator details.  The postgraduate database
    contains data on staff supervision of postgraduate students.  The CoS
    database contains data on the staff, and provides the
...
*** The system users
    Three users, the cluster staff, cluster
    leaders, and  overall administrator are identified.  From these
    three users are formed the three views.  Each user has different
    functionality.

**** Cluster user
     The staff who make up the cluster, and have a interest in making
     sure the data about them is correct.
**** Cluster leaders
     Who has the responsibility of collating the data for teaching
     allocations, and coordinator roles.  Also have a interest in
     seeing how their cluster is doing overall, and have the
     same interest as a ordinary staff member.
**** Administrator
     An administrator is not a academic staff member and is assumed to
     be a manager.  Sees the overall picture of how SEAT is doing
     broken down by cluster, campus and class.  Their role is to
     collate all the data from the clusters together, and also need
     to archive the current data, add new staff, and papers.

** Implementation
*** User Interface
    
**** Look and feel
    The idea was to make the pages have a report feel, but make parts
    of them editable.  This was overcome using jQuery, to alter the
    HTML DOM on the fly, using event triggers, such as click and hover on
    HTML elements.  This replaces a HTML element <span> with a
    <form> element of <input>. The <input> element is editable, and
    ...
    event is triggered, and uses Ajax, through jQuery, to connect
    with the server and send it the update, and the server then uses
...
#+CAPTION: The flow of events that happen when editing displayed data.
    ...
    The previous system was a spreadsheet, and the new system loosely
    models itself on this spreadsheet, or the tabular view.  The
    cluster view is an example of the spreadsheet model, shown in figure
    \ref{fig:redesign:clusterview}.  This mimics the view that is
...
[[file:images/staff_list-csit.png]]
    ...
    The cluster leader view is editable, modeled on a spreadsheet, with
    ...
    click the mouse on this cell, brings up a appropriate
    control, and the user edits the value with it.
...
remember the legal entry values.  When editing the
campus, coordinators, semester, mode, and staff in the teaching
allocations, a drop down menu appears when the element is
...
The controls, were all created from a
...
inserted into the HTML DOM.  This process takes place many times.
...
for finding a staff in this huge list is a problem.  The order of
the names is changed to help solve this problem, and are ordered
according to what cluster the names belong to, then ordered by surname.
This groups the staff according to what cluster
the staff belong to.  The menu entries are filled with first the cluster
the staff belong to, followed by their initials, then their first name,
...
out at a glance what cluster the staff belong to. Thus making finding a
...
is not always possible to give messages that tell the user exactly
what has gone wrong, but effort has gone into at least informing them
of a error.  Some actions also have confirm messages, that deal with lots of items at once, shown in figure
\ref{fig:cluster-commit}.  The red circles, labeled "1" and "2", are such
events, and label "1" resets
the information to the way it was before, and label "2" commits the changes to
the database.  Both nave confirm dialogs associated with them.
...
are corrected to equal a hundred, and that row will no longer be
red.
...
**** Three views
     When a user logs in, each see a different setup.
***** Cluster user
      ...
      The cluster leader also gets the chance to look at other staffs
      data, since this information is not secret across the groups.
***** Cluster leader
      This user sees a menu from which the user navigates to different staff
      ...
      The user also get to look at other clusters details.

      The other item in the menu is the cluster paper offering page.
      Here the user gets to edit the teaching allocations for the papers
      that any of their staff have something to do with.  Figure
      \ref{fig:cluster-commit} is a example of what a CSIT cluster
      leader sees in this page.

***** Administrator
     The administrator view sees a menu that also allows them to see
     the same views as a cluster leader, but instead of automatically
     being filtered by the cluster, and see all staff.  The cluster
     paper offerings view for them is there for those papers that have
     staff from two or more clusters.  This is needed to avoid
     concurrency problems and check to see if the data shown is correct,
     ...
     correct for all offerings.  A dialog will pop up when
     this is clicked to confirm the action taken.  The
     first of the duplicate offerings is used as the offering
     ...
     with the offering accepted, being copied to the main database.
...
*** Responsiveness
    ...
    possible.  Three bottle necks to speed of page loading exist.
    The server side scripts, the client side scripts, and the amount
    of data needed to process.  The server side scripts have the
    fastest execution time, and the client side Javascript, are
    slow, especially when inserting elements into the HTML DOM.
    It was decided the processing of data should be done
    ...
    amounts of data, such as filtering the staff list by cluster, campus
    or class.
*** Archiving of data
    The application allows the backing up of data to files.  These
    files are written in JSON format.  JSON is a formats that is
    written to or read by Django and makes it a the best choice for
    backup formats.   Data from a previous year, backed up, can
    be used to restore the database to where it was before the mistake
    took place.
*** Updating teaching allocation process
   ...
   paper offerings that their staff members are involved in, and not
   any other papers, and concurrency problems are avoided.  Their
   offerings that have staff from multiple clusters involved do cause problems.  To avoid conflicts a copy of the
   ...
   system.  If the offering has staff from that cluster only, 
   the offering will be copied to the original.  For those that
   involve multiple clusters, it is displayed in the
...
**** Python
     Other possible languages were PHP, Java or
     Perl. Python \cite{python1} is the language that we are
...
**** Django
   ...
   The MTV development pattern shown in figure \ref{fig:mtv}.
    - M is the model that is the data access layer.
    - T is the templates that is the presentation layer.
    - V is the views that is the business logic layer.
...
**** MySQL
     ...
     application.   The choice of this database was beyond the control
     of the developer.
**** Microsoft Access
     This is a external source that has been imposed on this
     application. It holds all the postgraduate supervision data,
     that is part of what makes up the final workload figure of
     staff.
**** ODBC
     ...
     called pyodbc \cite{pyodbc1}.  This hooks into unixODBC
     \cite{unixODBC}, that in turn uses a 
...
**** Javascript
     ...
     jQuery also helps overcome the
     incompatibilities over different browsers.
**** HTML and CSS
     ...
     a aid to laying out the HTML elements in the viewing browser.  To
     distribute this application as a web served application viewed by
     web browsers, HTML is needed.

  
** Testing
   Unit testing is difficult when testing user interactions.  Instead
   user testing was done.  Hans Guesgen, Stephen Marsland,
   Michelle Wagner, Giovanni Moretti and Patrick Rynhart, were
   voluntarily employed to test the application throughout
...
** Redesign phase
   ...
   the cluster leaders, and Glenda Kirk at a meeting.  The cluster
   leaders were given 
   a preview of the  application.


   The application tree view model prototype was not liked. It was
   discovered the cluster leaders liked the way that Microsoft Excel
   allowed you to edit the data about teaching allocations and paper
   coordination.

   A major redesign was undertaken for how the cluster leaders edit
   the teaching allocations and paper coordinations was undertaken.
   Peviously the interface had been based upon a tree 
   structure, with nodes showing the paper, that expand to show
   paper offerings and teaching allocations.  The cluster leaders,
   liked the way that the spreadsheet enabled the editing of the relevant
   sections.  The new interface gained some of
   the functionality of the spreadsheet. This is shown in figure
   \ref{fig:redesign:clusterview}. The old view is shown in figure
   \ref{fig:redesign:oldview}. 
...
*** Responsiveness
**** Data processing
Processing is done in two places, client side scripts, and on the
server side.  The more data processed on the server side,
reduces the capacity of the server.  Also doing it all on the server
side means the pages 
...
functionality required.   When compared with server side scripts, client side
scripts speeds are slow.  Javascript is the client side script used by
this application.

Since Javascript is slow, data processing by Javascript
was removed.  Javascript has been reduced to fetching small pieces of
data via Ajax, and displaying them in the page.  This means that the
processing is done by the server.  The time 
consuming bulk processing of data is reduced.  The pages get
redisplayed as little as possible, with the use of Ajax.  Mass
processing of data is done on the server side.  Use Javascript to process
or retrieve small amounts of data. 

**** Algorithm efficiency

The loading time of a page is influenced more by the
efficiency of the algorithm used, then the choice of programming
languages.  The loading time of a page was reduced by following the
following rules, based upon "Python Patterns - An Optimization Anecdote"
\cite{optimising1}.
...
Javascript is implemented differently across different
browsers. jQuery helps overcome, the compatibility issues
amongst different browsers, but there were rules that
need to be adhered to make sure your code works across the major
browsers.
  1) Always use var to declare variables.
  2) Avoid the use of a few keywords, such as delete for variable names.
  3) If creating a array list, do not put a comma after the last element.

The first and the third rules are for Internet Explorer, but the
second is true particularly for any browser based on Webkit, such as 
Safari.  With the second rule, some Javascript implementations
that, under certain circumstances, will allow you to use these keywords.

This application relies on Javascript, and if it is turned off then
hardly anything will work.  Nothing is done,  in response to
Javascript being disabled. In future, a warning message, telling the user this,
produced on the server side, should be shown.  Except showing a
message, if the browser has Javascript turned off, no other actions
are possible.
...
otherwise to vast to keep track of.

*** Concurrency / Multiple views
The concurrency issues have been avoided.  The possible
issue that will cause real problems, is if two cluster leaders from
the same cluster logged in and started editing the papers.  This
circumstance is unlikely to happen.  A possible
solution is to either implement concurrency solutions, such as locks, or
if two cluster leaders of the same cluster, logged on, to
logg both of them off with a warning message.

*** Look and feel of pages
Getting pages to look the same across browsers is difficult
when using the HTML markup.  The shape of the form controls are out
of your hand, and in browsers, cannot be manipulated easily
with CSS.  Firefox for example, uses native controls, such as checkboxes,
and does not allow CSS to alter the look of this control.  Methods
exist, sing CSS to make it look the way you want it, and are not
desirable, and add another unstable element into the 
page display.  

A way that was found to control the look of controls
was to simulate them, using HTML elements, such as the <img> element, along with Javascript
events to change something about this element, such as the image in the
<img> element.  This was done in the cluster view, in the
offered column.
...
*** Problems with Paper List page
    ...
    global coordinator being displayed for each paper.  This is
    expandable and shows the paper offerings along with the teaching
    ...
    the size of the output will be changed, and thus the browser will
    have to redisplay the page.  This often meant that the view will
    suddenly go somewhere else and the viewer will have to scroll up
    or down to find the expanded paper.  This part was fixed with the
    addition of href anchors.  This worked for expanding the paper
    view, but when it was collapsed, the page will scroll to a
...
*** Staff menu
    ...
    interested in staff from their own cluster.  The
    staff relevant to the cluster leader should appear at the
...
* Conclusion
   ...
   implemented successfully, to for full these needs.  The challenge of
   that each user seeing data in a way appropriate to them, have
   been meet through permissions, based upon which group the user
   belongs to, and has the access rights to interact with the data
   set as required.
  
   The programme planner was discussed, and a outline of how it will
   be implemented using constraints satisfaction was proposed.
   Since time constraints prevented the completion of this
   system, work still needs to be done.
