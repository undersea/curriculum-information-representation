...
* Implementation
** User Interface
    The major constraints of this project is the user interface.  The
    application is web served and the interaction takes place in a web
    browser.  The problem faced here is the incompatibilities across
    different browsers.  Finding HTML, CSS and Javascript which works in
    all major browsers is a challenge.  
...
#+CAPTION: The flow of events that happen when editing displayed data.
    ...
    Creating a interface which is usable and familiar to the users is the
    other challenge faced.  To create this interface a set of assumptions
    has to be made about the user of this system.  These are
     - They know how to navigate a site using hyperlinks.
     - They know how to fill out and submit on-line forms
     - They have used a spreadsheet before.
     - They need to be shown how to use the features, and what to look
       for.
    ...
    shown in figure \ref{fig:spreadsheet-teaching}.  Everything is
    grouped by category and where workload hours are being displayed,
    totals are shown at every stage.
...
clicked.  An example of this is shown in figure
\ref{fig:cluster-view-dropdown}.
...
Another fact about the controls, like the drop down menus used, is
that they were all created from a mixture of server side Python using
Django, Ajax and Javascript.  Javascript is called and uses Ajax to
connect and retrieve data from the server. The server side Python
script processes the request and returns a result, in the form of
JSON. This result is then evaluated by Javascript to form a HTML form
element. The form element is then inserted into the HTML DOM.  This
process could take place many times.  The process is shown in figure
\ref{fig:control-creation}.
...
according to what cluster they belong to, then ordered by surname.  This has the effect of grouping the
staff according to what cluster they belong to.  The menu entries are
filled with first the cluster they belong to, followed by their
initials, then their first name, followed by their last name. Making
it obvious what cluster each staff member belongs to in the drop down
menu makes it possible to figure out at a glance which cluster they
belong to. Thus making finding a particular staff from a cluster is
easier.  Figure \ref{fig:cluster-view-dropdown} gives a glimpse of
this.
...
How many teachers does a paper offering need?  This is a important question to
ask, when designing a user interface, which can add and remove
teachers.  Normally there is a maximum of three teachers
for each paper offering.  The decision was to only allow up to four
teachers per paper offering.  This helps overcome the layout problem,
as the entire listing of offerings is put in a 
