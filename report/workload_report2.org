...
* Introduction
  ...
  This is a tool to manage staff teaching allocations.  It shows how
  much they are doing and helps decide if they are doing to much or to
  little teaching.  Teaching allocations are their workload, thus this
  shows how much workload they are doing for each year.  From this
  data decisions can be made if staff can take on more work, or if
  they are taking on to much.

  With the large amount of data available for managing
  workloads of groups of staff in an organisation, a means to
  disseminate this data is needed.  Currently a spreadsheet
  application has been used to perform this task.  While the amount of
  data is small, using the spreadsheet worked.  This approach does not
  scale well. The means of gathering this information was painfully slow and time consuming.
  The spreadsheet application was also complicated and hard to use.  A
  new way of gathering the data together and disseminating the results
  has to be found.
