...
Django, Ajax and Javascript.  The Python side script returns text, in
the form of JSON, which is then evaluated by Javascript to form the
HTML form element, which are then inserted into the HTML DOM on
demand.  This process could take place many times, each time with
fresh data.  The process is shown in figure
...
teachers.  It seems that normally there is a maximum of three teachers
a paper offering has.  Using this information
